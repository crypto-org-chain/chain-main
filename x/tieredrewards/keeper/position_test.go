...
func (s *KeeperSuite) TestSetPosition_IncrementsValidatorCounter_NewDelegated() {
	...
	err = s.keeper.SetPosition(s.ctx, pos, &keeper.ValidatorUpdate{Previous: ""})
	s.Require().NoError(err)
...
func (s *KeeperSuite) TestSetPosition_SwapValidatorDecrementsAndIncrements() {
	...
	s.Require().NoError(s.keeper.SetPosition(s.ctx, pos, &keeper.ValidatorUpdate{Previous: ""}))
	...
	s.Require().NoError(s.keeper.SetPosition(s.ctx, pos, &keeper.ValidatorUpdate{Previous: valAddrA.String()}))
...
func (s *KeeperSuite) TestDeletePosition_RejectsWhileDelegated() {
	...
	err := s.keeper.DeletePosition(s.ctx, pos, &keeper.ValidatorUpdate{Previous: valAddr.String()})
