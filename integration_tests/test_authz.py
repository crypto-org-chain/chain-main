...
def test_execute_tx_within_authorization_spend_limit(cluster, tmp_path):
    """
    ...
    assert (
        cluster.balance(granter_address) == granter_initial_balance - transaction_coins
    )
    assert (
...
def test_execute_tx_beyond_authorization_spend_limit(cluster, tmp_path):
    """
...
def test_revoke_authorization(cluster, tmp_path):
    """
    ...
    revoke_authorization(cluster, grantee_address, SEND_MSG_TYPE_URL, granter_address)
    generated_tx_txt = tmp_path / "generated_tx.txt"
    generated_tx_msg = transfer(
        cluster,
        granter_address,
...
def test_generic_authorization_grant(cluster, tmp_path):
    """
...
def test_execute_delegate_to_allowed_validator(cluster, tmp_path):
    """
    ...
    granter_address = cluster.address("community")
    grantee_address = cluster.address("ecosystem")
    validator_initial_deligation_amount = int(
        query_delegation_amount(cluster, granter_address, allowed_validator_address)[
            "amount"
        ]
    )
    ...
    assert query_delegation_amount(
        ...
        "denom": BASECRO_DENOM,
        "amount": str(validator_initial_deligation_amount + delegate_coins),
    }

    # test execute delegate to other validators not in allowed validators should fail
...
def test_unable_to_execute_delegate_to_deny_validator(cluster, tmp_path):
    """
...
def test_execute_all_staking_operations(cluster, tmp_path):
    """
    ...
    validator1_initial_deligation_amount = int(
        query_delegation_amount(cluster, granter_address, validator1_address)["amount"]
    )
    validator2_initial_deligation_amount = int(
        query_delegation_amount(cluster, granter_address, validator2_address)["amount"]
    )
    ...
    assert query_delegation_amount(cluster, granter_address, validator1_address) == {
        "denom": BASECRO_DENOM,
    ...
    assert query_delegation_amount(cluster, granter_address, validator2_address) == {
        "denom": BASECRO_DENOM,
