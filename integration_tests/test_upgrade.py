...
def test_manual_export(cosmovisor_cluster):
    ...
    newGenesisTime = (datetime.utcnow() \
            + timedelta(seconds=5)).replace(tzinfo=None).isoformat("T") \
            + "Z"

    cluster.config["genesis-time"] = newGenesisTime

    for i in range(cluster.nodes_len()):
        cluster.overrideGenesisTime(i)
