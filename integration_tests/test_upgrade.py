...
def test_manual_upgrade_all(cosmovisor_cluster):
    ...
    assert vesting_acct['original_vesting'] == [{'denom': 'basecro', 'amount': '7000000000000000000'}]
    assert len(acct['vesting_periods']) == 60
    for period in acct['vesting_periods'][:-1]:
        assert period == {'length': '60', 'amount': [{'denom': 'basecro', 'amount': '116666666666666666'}]}
    assert acct['vesting_periods'][-1] == {'length': '60', 'amount': [{'denom': 'basecro', 'amount': '116666666666666706'}]}

    params = json.loads(cli.raw("query", "mint", "params", output="json", node=cli.node_rpc))
