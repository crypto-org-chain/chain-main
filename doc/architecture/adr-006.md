...
Allow users to **lock tokens into a tier** (e.g. 1, 2, or 5 years). Locked tokens are **internal to the tier mechanism**: they can only be delegated or redelegated **within** the tier module (undelegation only after the exit commitment has elapsed); (like internal liquid-stake tokens that cannot be used outside this system). **Locking always delegates to a validator** — there is no locking without delegation. This ensures tier lockers provide security to the network from the moment they lock. Users receive:
...
| **Tier position** | A record stored in module state: unique position ID, owner, tier_id, **amount_locked**, optional exit triggered time and exit unlock time, validator and **delegated shares**. A position is always delegated at creation. A position **cannot be broken down**: the full amount is delegated to a single validator as a whole (all-or-nothing). |
...
User -> MsgLockTier(tier_id, amount, validator [, trigger_exit_immediately])
  ...
  -> Set TierPosition: position_id, owner, tier_id, amount_locked=amount, CreatedAtHeight, CreatedAtTime,
       ExitTriggeredAt=0, ExitUnlockTime=0, Validator=validator, DelegatedShares=newShares,
...
- **Rewards Pool:** `RewardsPoolBalance` - balance of the rewards pool.
