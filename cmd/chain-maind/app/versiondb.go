...
func VersionDBChangeSetGroupCmd(opts versiondbclient.Options) *cobra.Command {
	...
	cmd.AddCommand(
		...
		FixVersionDB(opts),
		DumpZeroVersionDBChangeSet(opts),
	)
	return cmd
}
...
func DumpZeroVersionDBChangeSet(opts versiondbclient.Options) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "dump-zeroversiondb [dir] [outDir]",
		Short: "dump zeroversiondb changeset at version [dir] [outDir]",
		Args:  cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			dir := args[0]
			outDir := args[1]
			...
			for _, storeKey := range opts.DefaultStores {
				...
				var pairs []*iavl.KVPair
				for ; it.Valid(); it.Next() {
					if binary.LittleEndian.Uint64(it.Timestamp()) != 0 {
						continue
					}
					key := make([]byte, len(it.Key()))
					copy(key, it.Key())
