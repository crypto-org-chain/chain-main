...
// GenTxCmd builds the application's gentx command. (converts CRO to Carson)
// nolint: errcheck
func CustomGenTxCmd(ctx *server.Context, cdc *codec.Codec, mbm module.BasicManager, smbh StakingMsgBuildingHelpers,
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			nmsg := msg.(staking.MsgCreateValidator)
			coin, err := sdk.ConvertCoin(nmsg.Value, app.BaseCoinUnit)
			if err != nil {
				return fmt.Errorf("failed to convert coins: %w", err)
			}
			nmsg.Value = coin
